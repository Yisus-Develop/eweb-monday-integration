<?php
// webhook-handler-with-group-movement.php
// Webhook handler con movimiento automático de grupos según Lead Score
...
try {
    ...
    // Determinar el grupo según el Lead Score
    $targetGroupId = 'topics'; // Grupo por defecto
    if ($scoreResult['total'] > 20) {
        $targetGroupId = 'group_mkypkk91'; // HOT Leads (Score > 20)
    } elseif ($scoreResult['total'] >= 10) {
        $targetGroupId = 'group_mkypjxfw'; // WARM Leads (Score 10-20)
    } elseif ($scoreResult['total'] < 10) {
        $targetGroupId = 'group_mkypvwd'; // COLD Leads (Score < 10)
    }

    // Si es un email desechable, mover a grupo de spam
    if ($isDisposable) {
        $targetGroupId = 'group_mkyph1ky'; // Spam - Revisar
    }
    ...
    if (!empty($existingItems)) {
        ...
        // Mover el ítem al grupo correspondiente según su nuevo score
        try {
            $monday->moveItemToGroup($itemIdToUpdate, $targetGroupId);
        } catch (Exception $e) {
            error_log("Error moviendo ítem al grupo: " . $e->getMessage());
        }
    } else {
        ...
        // Mover el ítem recién creado al grupo correspondiente según su score
        if ($monday_id) {
            try {
                $monday->moveItemToGroup($monday_id, $targetGroupId);
            } catch (Exception $e) {
                error_log("Error moviendo ítem al grupo: " . $e->getMessage());
            }
        }
